...
Destination-specific Responses sanitization, including removal of the ChatGPT-only top-level
`access_programs` field on noncanonical routes, is owned by the [Responses transport](transports/responses.md#responses-httpsse).
...
| `src/diagnostics/` | Bounded, secret-safe transaction evidence and support-export assembly. Transport, request, credential-source, retry, and recovery observations are correlated without retaining request bodies or raw credentials. |
...
| Evidence and contracts | `src/compatibility/`, `src/diagnostics/`, `src/lab/` |
...
`src/adapters/openai-responses.ts` omits top-level `user` and `safety_identifier` only at the canonical ChatGPT Codex forward destination, whose strict allowlist rejects both public identity fields. Claude translation retains its original identity and prompt-cache key; public API and noncanonical gateways retain caller-provided `user` and `safety_identifier` fields. Input roles, tool-schema properties and original replay bodies are not changed.
