...
- **INV-TOML-01** — Root TOML keys such as `model_provider` must stay before any table;
  user-owned custom `model_catalog_json` overrides are preserved at the root, but OpenCodex does
  not add one.
