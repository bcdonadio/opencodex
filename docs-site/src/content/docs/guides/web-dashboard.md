...
Request transport metadata distinguishes the client connection from the provider connection.
`inboundTransport` is `http` or `websocket`; `upstreamTransport` is `http`, `websocket`, or
`mixed` when actual upstream request sends used both transports across retries. An unsuccessful
WebSocket upgrade followed by an HTTP request records `http`. Streaming HTTP responses (SSE)
remain HTTP; a WebSocket response internally adapted to SSE remains WebSocket.

`accountLogLabel` identifies the selected OpenAI/Codex account as `main` or its stable opaque
Pool label, such as `p3fa91c`. Recorded attempt details retain their account and transport;
an attempt may include multiple physical sends, and its account label identifies the last
selected account. The dashboard can show a current configured nickname alongside the stable label.
Labels do not contain account emails, raw account IDs, or credentials. Historical entries and
requests that sent nothing upstream may have no recorded transport or account; missing values
are not inferred from the current settings. `ocx logs` also prints recorded transport and account
metadata, and `ocx logs --json` exposes the structured fields.
