...
import { scrubOcxCompactionItems, stripCanonicalOnlyToolFields, stripCanonicalOnlyTopLevelFields, stripChatGptAccessPrograms, stripInternalChatMessageMetadataPassthrough, stripInvalidItemIds, stripItemIdsWhenUnstored } from "./request-strips";
...
export function createResponsesPassthroughAdapter(provider: OcxProviderConfig): ProviderAdapter & { passthrough: true } {
  return {
    ...
    buildRequest(parsed: OcxParsedRequest, incoming: IncomingMeta) {
      ...
      if (!isCanonicalOpenAiForwardProvider(provider)) {
        outBody = stripChatGptAccessPrograms(outBody);
