...
import {
  bindEphemeralReplayStore,
  copyEphemeralReplayProvenance,
  markEphemeralReplayBody,
...
export function expandPreviousResponseInput(
  ...
  ensureLoaded();
  pruneResponses();
  const ephemeral = resolveEphemeralReplay(previousId, ephemeralScope);
  if (ephemeral.kind === "scope_mismatch") {
  ...
  const previous = ephemeral.kind === "hit" ? ephemeral.state : states.get(previousId);
  if (!previous) return body;
  const materialized = ephemeral.kind === "hit"
    ? { ok: true as const, state: ephemeral.state }
  ...
  // has none simply never skips.
  {
    ...
    if (
      ...
      if (ephemeral.kind === "hit") markEphemeralReplayBody(unchanged, ephemeral.expiresAt);
      return unchanged;
    }
  }
  const expanded = {
