...
function releaseDrainedCodexAccountPin(
  config: OcxConfig,
  selectionOptions?: Pick<CodexAccountUsabilityOptions, "nativeMainSelectionOnly" | "isMainAccountTokenLive" | "callerBackedMainSelection" | "preserveCallerMainPin">,
  now: number = Date.now(),
): void {
...
export function resolveCodexAccountForThreadDetailed(
  ...
  const publishSharedState = selectionOptions?.suppressSharedStateMutations !== true;
  ...
  const strategyPick = pickUnboundStrategyAccount(
    ...
    !modelScopedSelection && publishSharedState,
    !preserveExistingModelScopedAffinity && publishSharedState,
  );
  if (strategyPick) {
    if (threadId && preserveExistingModelScopedAffinity && publishSharedState) {
  ...
  if (preempted) {
    ...
    if (
      ...
      if (
        publishSharedState
        && !manualPreferenceBlocks(POOL_KEY_CODEX, preempted)
      ) {
  ...
  active = applyQuotaAutoSwitch(
    ...
    quotaScope,
    selectionOptions,
    !preserveSharedSelectionForModelDetour && publishSharedState,
  );
  ...
  if (threadId && publishSharedState) {
