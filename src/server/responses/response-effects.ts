...
import { recordContextTransformation } from "../transaction-capture";

/** Owns completion callbacks, replay publication, and live tool aliases. */
export function createResponsesEffects(
  requestContext: Pick<ResponsesRequestContext, "options" | "req" | "config" | "logCtx">,
  ...
  const { options, req, config, logCtx } = requestContext;
