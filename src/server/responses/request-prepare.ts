...
import type {
  AgentTaskRecoveryDeliveryResult,
  AgentTaskRecoveryFailureReason,
} from "./agent-task-recovery";
...
import {
  recordContextTransformation,
  recordReconstructedContext,
  recordRequestShape,
  recordRequestedReasoning,
  recordSelectedRoute,
...
export async function prepareResponsesRequest(
  ...
  if (agentTaskRecoveryTraceId && !agentTaskRecoveryGateReason && agentTaskRecovery) {
    let recoveryResult: AgentTaskRecoveryDeliveryResult = { recovered: false };
    ...
    try {
      recoveryResult = await recoverEncryptedAgentTaskForDelivery(
        req,
        (body as { input?: unknown } | undefined)?.input,
        agentTaskRecovery,
        config,
      ...
      recoveryFailureReason = recoveryResult.recovered ? undefined : recoveryResult.reason;
    } catch {
    ...
    if (recoveryResult.recovered) {
      options.recoveryDeliveryTestHook?.(
        (body as { input?: unknown } | undefined)?.input,
        recoveryResult,
      );
    }
      ...
      if (!unreadableEncryptedAgentTask) {
        try {
          ...
          if (
            ...
            bindTurnTerminationScope(nativeReparse, resolvedConversationId);
  ...
  recordRouteAuth(logCtx, route.provider.authMode, admissionState.authCtx);
