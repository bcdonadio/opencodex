...
/** Capture caller-owned credential facts once, before combo/WS request reconstruction. */
export function captureResponsesIngressCredentialOwnership(
  headers: Headers,
  config: OcxConfig,
  options: HandleResponsesOptions,
): Pick<HandleResponsesOptions,
...
> {
  const requestScopedMainCredential = options.requestScopedMainCredential
    ?? (options.admission?.source !== "bearer" && hasForwardableCodexBearer(headers, config));
  const inboundBearer = headers.get("authorization")?.replace(/^Bearer\s+/i, "").trim() ?? "";
  return {
    requestScopedMainCredential,
    authorizationBearerIsProxySecret: options.authorizationBearerIsProxySecret
      ?? (!!inboundBearer && isProxyAdmissionSecret(inboundBearer, config)),
    authorizationBearerIsDataPlaneSecret: options.authorizationBearerIsDataPlaneSecret
...
export function codexRouteCredentialOwnership(
  ...
  const substituteMainCredential = (
    ...
    || (canonicalForwardTransport && options.authorizationBearerIsProxySecret === true)
  ) && (route.codexAccountMode !== undefined || canonicalForwardTransport);
  return {
    substituteMainCredential,
    requestScopedMainCredential: (options.requestScopedMainCredential
      ?? hasForwardableCodexBearer(authInputHeaders, config)) === true
      && route.codexAccountMode !== undefined
      && route.codexAccountId === undefined
      && canonicalForwardTransport
      && !substituteMainCredential
  };
}
...
> {
  const { req, config, route, authCtx, substituteMainCredential, options } = args;
  if (authCtx.kind !== "main-pool" || !("accessToken" in authCtx)) {
