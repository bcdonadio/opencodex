...
import { recordReconstructedContext } from "../transaction-capture";
...
export async function executeComboResponses(
  ...
  const body = expandPreviousResponseInput(
    rawBody,
  ...
  const recoverUnreadableEncryptedTask = async (): Promise<boolean> => {
    ...
    if (!verifyRecoveredAgentTaskDelivery(
      (body as { input?: unknown } | undefined)?.input,
      recoveryResult,
    )) {
      discardRecoveryCache();
      ...
      recoveryFailureReason = "input_changed";
      return false;
    }
    logAgentTaskRecoveryDiagnostic({
      traceId: recoveryTraceId,
